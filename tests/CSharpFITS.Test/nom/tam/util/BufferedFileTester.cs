...
namespace nom.tam.util
{
    ...
    public class BufferedFileTester
    {
        ...
        public void TestBufferedFile()
        {
            ...
            if (args.Length > 1)
            {
                dim = Int32.Parse(args[1]);
            }
            int numIts = 1;
            if (args.Length > 2)
            {
                numIts = Int32.Parse(args[2]);
            }

            Console.Out.WriteLine("Allocating arrays.");
            ...
            Console.Out.WriteLine("Initializing arrays -- may take a while");
            int sign = 1;
            for (int i = 0; i < dim; i += 1)
            {
                double x = sign * Math.Pow(10.0, 20 * SupportClass.Random.NextDouble() - 10);
                db[i] = x;
                fl[i] = (float)x;
            ...
            sh[1] = Int16.MinValue;
            sh[0] = 0;
            in_Renamed[0] = Int32.MaxValue;
            in_Renamed[1] = Int32.MinValue;
            in_Renamed[2] = 0;
            ln[0] = Int64.MinValue;
            ln[1] = Int64.MaxValue;
            ln[2] = 0;
            fl[0] = Single.Epsilon;
            fl[1] = Single.MaxValue;
            fl[2] = Single.PositiveInfinity;
            fl[3] = Single.NegativeInfinity;
            fl[4] = Single.NaN;
            fl[5] = 0;
            db[0] = Double.MinValue;
            db[1] = Double.MaxValue;
            db[2] = Double.PositiveInfinity;
            db[3] = Double.NegativeInfinity;
            db[4] = Double.NaN;
            db[5] = 0;
        ...
        public static void standardFileTest(String filename, int numIts, int[] in_Renamed, int[] in2)
        {
            ...
            for (int j = 0; j < numIts; j += 1)
            {
                for (int i = 0; i < dim; i += 1)
                {
                    var temp_BinaryReader = new BinaryReader(f);
            ...
            lock (f)
            {
                ...
                Console.Out.WriteLine($"  SyncRAF Int write: {(4 * dim * numIts) / (1000 * deltaTime())}");
                f.Seek(0, SeekOrigin.Begin);
                resetTime();
                for (int j = 0; j < numIts; j += 1)
                {
                    for (int i = 0; i < dim; i += 1)
                    {
                        var temp_BinaryReader2 = new BinaryReader(f);
        ...
        public static void standardStreamTest(String filename, int numIts, int[] in_Renamed, int[] in2)
        {
            Console.Out.WriteLine("Standard I/O library");
        ...
        public static void buffStreamSimpleTest(String filename, int numIts, int[] in_Renamed, int[] in2)
        {
        ...
        public static void bufferedStreamTest(String filename, int numIts, double[] db, double[] db2, float[] fl, float[] fl2, long[] ln, long[] ln2, int[] in_Renamed, int[] in2, short[] sh, short[] sh2, char[] ch, char[] ch2, byte[] by, byte[] by2, bool[] bl, bool[] bl2, int[][][][] multi, int[][][][] multi2)
        {
...

            {
                ...
                for (int i = 0; i < numIts; i += 1)
                    f.ReadArray(db2);
                Console.Out.WriteLine($"  BDS Dbl read:  {(8 * dim * numIts) / (1000 * deltaTime())}");
                for (int i = 0; i < numIts; i += 1)
                    f.ReadArray(fl2);
                Console.Out.WriteLine($"  BDS Flt read:  {(4 * dim * numIts) / (1000 * deltaTime())}");
                for (int i = 0; i < numIts; i += 1)
                    f.ReadArray(in2);
                Console.Out.WriteLine($"  BDS Int read:  {(4 * dim * numIts) / (1000 * deltaTime())}");
                for (int i = 0; i < numIts; i += 1)
                    f.ReadArray(ln2);
                Console.Out.WriteLine($"  BDS Lng read:  {(8 * dim * numIts) / (1000 * deltaTime())}");
                for (int i = 0; i < numIts; i += 1)
                    f.ReadArray(sh2);
                Console.Out.WriteLine($"  BDS Sht read:  {(2 * dim * numIts) / (1000 * deltaTime())}");
                for (int i = 0; i < numIts; i += 1)
                    f.ReadArray(ch2);
                ...
                for (int i = 0; i < numIts; i += 1)
                    f.ReadArray(bl2);
                Console.Out.WriteLine($"  BDS Boo read:  {(1 * dim * numIts) / (1000 * deltaTime())}");
                ...
                for (int i = 0; i < 5; i += 1)
                {
                    ...
                    var temp_Int64 =
                        //temp_BinaryReader = f;
                        f.Position; //temp_BinaryReader.BaseStream.Position;
        ...
        public static void bufferedFileTest(String filename, int numIts, double[] db, double[] db2, float[] fl, float[] fl2, long[] ln, long[] ln2, int[] in_Renamed, int[] in2, short[] sh, short[] sh2, char[] ch, char[] ch2, byte[] by, byte[] by2, bool[] bl, bool[] bl2, int[][][][] multi, int[][][][] multi2)
        {
            int dim = db.Length;
            ...
            var bs2 = (byte)f.ReadByte();
            var cs2 = f.ReadChar();
            var ss2 = f.ReadInt16();
            var is2 = f.ReadInt32();
            var ls2 = f.ReadInt64();
            var fs2 = f.ReadSingle();
            var ds2 = f.ReadDouble();
            var bls2 = f.ReadBoolean();

            // Now read only pieces of the multidimensional array.
            for (int i = 0; i < 5; i += 1)
            {
                // Skip the odd initial indices and read the evens.
                var temp_Int64 = f.Position;
